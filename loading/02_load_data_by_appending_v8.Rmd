...
```{r}
config = read_csv('D:/Data/Processed/archive/combined_date_complied_rectified_num_config.csv')
combined = readRDS("D:/Data/Processed/archive/combined_date_complied_rectified_num_data.rds")
...
```{r}
raw = read_csv("D:/Data/Flat_File_v8/BGN_1158015_pat_data.csv",
...
# pick one patient and check
index = which(raw$pat_id %in% raw_dup$pat_id[1])
raw__ = as.data.frame(cbind( t(raw_dup[1,]), t(raw[index,])))
sum(raw__$V1 == raw__$V2, na.rm = TRUE)

length(unique(combined$pat_id))
length(unique(raw$pat_id))

length(setdiff(unique(combined$pat_id), unique(raw$pat_id)))

#matches_ = combined$pat_id %in% raw$pat_id

colnames_raw = colnames(raw)
var_additional = colnames_raw [ str_detect(colnames_raw, 'gaba') ]
#var_additional = setdiff(colnames(raw), colnames(combined))
```
## try combining them
...
saveRDS(combined_new, "D:/Data/Processed/combined_date_complied_rectified_num_gaba_data.rds")
saveRDS(config,       "D:/Data/Processed/archive/combined_date_complied_rectified_num_gaba_config.rds")

write_csv(combined_new, "D:/Data/Processed/archive/combined_date_complied_rectified_num_gaba_data.csv")
write_csv(config,       "D:/Data/Processed/combined_date_complied_rectified_num_gaba_config.csv")
