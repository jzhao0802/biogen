...
corr = cbind(combi__, as_tibble(out_))
corr$Description_v1 = Description[[colnames_[corr$`1`]]]
corr$Description_v2 = Description[[colnames_[corr$`2`]]]

out_ = as.data.frame(unlist(apply(combi__, 1,is.na_pair)))
colnames(out_) = 'Count'
corr$Count = out_$Count
corr$Completeness = out_$Count / max(out_$Count)

colname__ = c("1", "2", "Completeness", "Count", "pearson_corr", "pearson_p.value" , "spearman_rho", "spearman_p.value", "Description_v1", "Description_v2")
corr = corr %>% select(one_of(colname__))
saveRDS(corr, 'D:/Data/QC_v8/corr.RDS')
corr = readRDS('D:/Data/QC_v8/corr.RDS')
write_csv(corr, 'D:/Data/QC_v8/corr.csv')
```
