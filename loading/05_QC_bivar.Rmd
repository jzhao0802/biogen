...
var2ignore = config$Column[ config$Type=="others" |  
  str_detect(config$Column, "npi|age_group|hce_id|pat_id|flg") ]
...
```
# run the program -- this will take some times
...
corr = cbind(out_, table__)
corr = corr %>% select(one_of(c('Count', 'Completeness')), everything())

#add unique Count
nUnique_ = (as.data.frame(sapply(data_,function (x) {length(unique(x))})))
colnames(nUnique_) = 'value'

corr$`Number of unique values in V1` = nUnique_$value[corr$V1]
corr$`Number of unique values in V2` = nUnique_$value[corr$V2]

corr$V1_name = colnames_[corr$V1]
corr$V2_name = colnames_[corr$V2]

saveRDS(corr, 'D:/Data/QC_v8/corr_v2.RDS')
corr = readRDS('D:/Data/QC_v8/corr_v2.RDS')
write_csv(corr, 'D:/Data/QC_v8/corr_v2.csv')

write_csv(corr %>% filter(!is.na(corr$pearson_corr)), 'D:/Data/QC_v8/corr_v2_noNA.csv')

# further filter on variables actually used for prediction
config = read_csv("D:/Data/Processed/combined_date_complied_rectified_num_gaba_copay_config.csv")
var2use = config$Column [ config$v1 | config$v2 | config$v3]

selected = corr$V1_name %in% var2use & corr$V2_name %in% var2use
write_csv(corr %>% 
  filter(!is.na(corr$pearson_corr)) %>% 
  filter(V1_name %in% var2use & V2_name %in% var2use), 
  'D:/Data/QC_v8/corr_v2_noNA_var2use.csv')
```

## Check
```{r}
no_answer = which(is.na(corr$pearson_corr))
#debug(correlation)
pair = combi__[no_answer,]
correlation(pair[1,])


cbind(nUnique_$value[pair[,1]], nUnique_$value[pair[,2]])

for (j in 1:15) {
  #psych::scatterHist(x=data_[,pair[j,1]], y=data_[,pair[j,2]], xlab=colnames_[pair[j,1]], ylab=colnames_[pair[j,2]])
  plot(x=data_[,pair[j,1]], y=data_[,pair[j,2]], xlab=colnames_[pair[j,1]], ylab=colnames_[pair[j,2]])
}
