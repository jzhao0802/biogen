...
config = readRDS("D:/Data/Processed/combined_date_config.rds")
...
config = read_csv("D:/Data/Processed/combined_date_config.csv")
...
for (j in 1:length(indices)) {
  var_index = indices[j]
  print( sprintf("%s", colnames_[indices[j]]))
  final_config$Description[indices[j]]
  ...
  count_stat[[j]] = plot_cond_density(combined, 
    ...
    target_label = 'discon',
    description = config$Description[indices[j]],
...
write_csv(eer_table, 'num_eer_analysis.csv')
...
write_csv(config_, "var_config_numeric.csv")
