...
# checking
config__ = config %>% dplyr::select( one_of(paste0('v',0:4)) )
...
combined_ = combined %>% dplyr::select( -one_of(var2eliminate))
...
stat_ = as.data.frame(sapply(config_ %>% dplyr::select(paste0('v',0:4)), sum))
...
combined_ = combined %>% dplyr::select( one_of(
  union_all('discontinue_flg','pat_id',var2use[[i]])))
dim(config_)
dim(combined_)
...
if (length(selected_num)>0) {
  ...
  combined_num = extreme_values(
      combined_  %>% dplyr::select( one_of(config_$Column[c(key,selected_num)])),
      var_config = paste0(results_dir_, 'modelling_var_config_num.csv'),
      pth = 0.99,
      output_dir = results_dir_)
  combined_num = combined_num$data %>%   dplyr::select( - one_of('pat_id'))
} else {
  combined_num=combined_ %>% dplyr::select( one_of('dodo'))
}
...
if (length(selected_cat)>0) {
  ...
  combined_cat = dummy_vars(combined_ %>% 
    dplyr::select( one_of(config_$Column[c(key,selected_cat)])),
    var_config = paste0(results_dir_, 'modelling_var_config_cat.csv'),
    output_dir = results_dir_)
    
  combined_cat = combined_cat$dummyfied %>%   dplyr::select( - one_of('pat_id'))
} else {
  combined_cat=combined_ %>% dplyr::select( one_of('dodo'))
}

#integer variable
combined_int = combined_ %>% dplyr::select( one_of(config_$Column[c(selected_int)]))
...
for (k in 1:5) {
  dataset = makeClassifTask(id='full', data=data[kf!=k,], target='label', positive=1)
...
which(is.na(column_))
logit_xgboost = logit_transform(column_)
saveRDS(logit_xgboost, 'D:/Results/modelling_09_bayesian/logit_xgboost.rds')
...
mat_ = mat_ %>% dplyr::select(one_of('rowname'),everything())
rownames(mat_)= NULL
