...
# function to generate posterior probability of discontinuation given the feature.
predict_bayes <- function(model, feature) {
  scores = model$llh1(feature) /(model$llh1(feature) + model$llh0(feature))
  # score = likelihood_1 / unconditional distribution of feature
  return(scores)
}

# function to spit out model scores and likelihood over a feature space
...
# this is just for testing the pipeline on one feature
...
# taking an individual feature vector as an example (abdominal pain event)
# and removing NAs
...
# training an LR to get weights for abdominal pain event (also training on
# a constant 1)
...
# running the LR on feature space (again including a constant column of 1s)
prob_lr = predict(glm_, cbind(x, 1), type='response', s=0.01)

# mat is a matrix of the feature space and the LR probabilities for the feature space.
mat =  rbind( mat__,
...
for (m in 2:3) {
  ...
  # select all the date diffs which are only in model 2:
  varlist_m = config$Column[ config[, paste0('v', m)] == TRUE ]
  varlist_m = varlist_m [ str_detect(varlist_m, '_diff') ]
  
  # the variable for which we have a lot of data (i.e. everything only in model 2
  # not in the Bayes analysis)
  varlist_m = setdiff(varlist_m, vlist[[m]])
  
  #collect all the data into one vector
